...
class Post_model extends CI_Model
{
    ...
    public function input_values()
    {
        $data = array(
            ...
            'title_slug' => $this->input->post('title_slug', false),
    ...
    public function add_post()
    {
        ...
        //$data["title_slug"] = str_slug($data["title"]);
		 $data["title_slug"] = sanitize($data["title"]);
        $data["slider_order"] = 0;
        if (!$data["is_slider"]) {
            $data["is_slider"] = 0;
        }
    ...
    public function get_posts()
    {
		...
		//$this->db->where('post.category_id = 10');
    ...
    public function get_paginated_posts($per_page, $offset)
    {
        ...
        $this->db->select('posts.* , categories.name as category_name');
        $this->db->where('posts.visibility', 1);
		//$this->db->group_by('posts.category_id');
        $this->db->order_by('posts.id', 'DESC');
    ...
    public function get_category_posts($category_id)
    {
        ...
        $this->db->from('posts');
        $this->db->join('categories', 'posts.category_id = categories.id'); 
        $this->db->where_in('posts.category_id', $category_id);
		$this->db->order_by('posts.id', 'DESC');
		$this->db->limit(8);
        $query = $this->db->get();
        return $query->result();
